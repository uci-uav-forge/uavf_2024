...
class ParticleFilter:
    def __init__(self, resolution: tuple[int,int], focal_len_pixels: float, num_particles: int = 1000, missed_detection_weight: float = 1e-4, pos_noise_std: float = 1, vel_noise_std: float = 0.1, radius_noise_std: float = 0.01):
    ...
    def update(self, cam_pose: tuple[np.ndarray, Rotation], measurements: list[np.ndarray]):
        '''
        ...
        for measurement in measurements:
            ...
            w, h = x2-x1, y2-y1
            bbox_r = (w+h)/2
            initial_direction_vector = np.array([x-self.resolution[0]/2,self.resolution[1]/2-y,-self.focal_len_pixels])
            ...
            for radius in np.linspace(0.1,1,10):
                ...
                d = radius * self.focal_len_pixels / bbox_r
